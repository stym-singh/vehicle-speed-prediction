				...
				if len(vehicle.trace) > 1:
					...
					try:
						...
						if vehicle.passed:
							...
							#cv2.putText(frame, 'MPH: %s' % int(random.randrange(abs(HIGHWAY_SPEED_LIMIT-25), HIGHWAY_SPEED_LIMIT-10, 2)), (int(trace_x), int(trace_y)), font, 1, (0, 255, 255), 1, cv2.LINE_AA)
							cv2.putText(frame, 'MPH: %s' % int(vehicle.mph), (int(trace_x), int(trace_y)), font, 1, (0, 255, 255), 1, cv2.LINE_AA)
					except:
						pass
